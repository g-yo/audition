...
def movie_add(request):
    if request.method == 'POST':
        form = MovieForm(request.POST, request.FILES)  # Include request.FILES to handle file uploads
        if form.is_valid():
            form.save()
            return redirect('movie_list')  # Adjust 'movie_list' to your actual movie list view name
    else:
        form = MovieForm()
...
def delete_movie(request, movie_id):
    ...
    if request.method == "POST":
        movie.delete()
        return redirect('movie_list')  # Replace 'movie_list' with your actual movie list view name
